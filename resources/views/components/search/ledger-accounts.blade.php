        ...
        <div wire:ignore>
            <select id="{{ $jsId }}" @if (isset($attributes['multiple']) && $attributes['multiple']) multiple @endif
                ...
                @foreach ($ledgeraccounts as $ledgeraccount)
                    <option value="{{$ledgeraccount->id}}">{{$ledgeraccount->ledger_code}} | {{$ledgeraccount->name}} </option>
                @endforeach
            </select>
        </div>
...
@push('js')
    <script>
        var config{{ $jsId }} = {
            valueField: 'id',
            labelField: 'name',
            searchField: ['id', 'ledger_code', 'name'],
            load: (query, callback) => {
                axios.get("{{ route('backend.search', ['type' => 'ledgeraccounts']) }}", {
                        params: {
                            search: query
                        }
                    })
            ...
            render: {
                    ...
                    return `<div class="py-1 d-flex">
                                <div>
                                    <div>
                                        ...
                                        <span class="text-muted"> | ${ escape(item.ledger_code) }</span>
                                    </div>
                ...
                item: function(item, escape) {
                    return `<div> ${ escape(item.ledger_code) } | ${ escape(item.name) }</div>`;
                }
            },
            loadThrottle: 1000
        };
        ...
        document.addEventListener("livewire:navigated", function(event) {
            ...
            window.addEventListener("{{ $attributes->wire('model')->value }}_update", event => {
                select{{ $jsId }}.addOption({
                    ...
                    ledger_code: event.detail.ledger_code,
