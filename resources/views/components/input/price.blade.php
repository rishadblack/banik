<div  class="form-group">
    ...
    <div class="input-group">
        <input @if($attributes['read-only']) readonly @endif {{$attributes->wire('model')}} onFocus="this.select()" type="number" id="{{$attributes->wire('model')->value}}" class="form-control @error($attributes->wire('model')->value) invalid @enderror {{ $attributes['class'] }}"  placeholder="{{ isset($attributes['placeholder']) ? $attributes['placeholder'] : 'Please Type '.$attributes['label']  }}">
