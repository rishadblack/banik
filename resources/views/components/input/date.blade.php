...
<div class="mt-2">
    ...
    @isset($attributes['require'])
            ...
            @endisset
    <div wire:ignore>
        <input x-data x-init="flatpickr($refs.{{$attributes->wire('model')->value}}, {{json_encode((object)$options)}});" {{ $attributes->wire('model') }} x-ref="{{$attributes->wire('model')->value}}" @if($attributes['read-only']) readonly @endif type="text" id="{{$attributes->wire('model')->value}}" class="form-control  @error($attributes->wire('model')->value) is-invalid @enderror {{$attributes['class']}}" placeholder="{{ $attributes['placeholder'] }}">
    </div>
